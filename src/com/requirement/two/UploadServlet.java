...
import com.resize.util.ResizeServlet;
...
public class UploadServlet extends HttpServlet{
	 ...
	 try{
	           ...
	           if(item.isFormField()){
	        	   ...
	        	   if(name.equals("toDirectory")) {
		       	    	...
		       	    	if(Streams.asString(stream).equals("saveToLocal")) {
		       	    		...
		       	    		System.out.println("Masuuuuukkkkk 1");
		       		   		Global.isLocal = true;

		       	   		} else {

		       	    		System.out.println("Masuuuuukkkkk 2");
                    ...
                    Global.fileName = new File(item.getName()).getName();
        ...
        if(Global.isLocal) {
    	    ...
    	    while ((read = in.read(buffer)) != -1) {
	        	...
	        	Global.percentDone = (int) Math.floor((100.00 * (double)byteTransferred/(double)bytes.length));
	        	System.out.println("percentDone : " + Global.percentDone);

    	    }
    	    	ResizeServlet.setSize(compile(bytes), localPath+"/", Global.fileName);
	...
	private InputStream compile(byte[] baos) {
		return new ByteArrayInputStream(baos);
	}
